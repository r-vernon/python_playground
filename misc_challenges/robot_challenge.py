...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan  8 17:26:08 2020
...
def upDir(currDir,cmd):
    return (currDir + ((cmd*2)+3) - 1)%4 + 1

mvDir = {1:(-1,0), 2:(0,1), 3:(1,0), 4:(0,-1)}

currPos = [0,0]
currDir = 2
currMv = mvDir[currDir]

for cmd in commands:
    
    if cmd < 1:
        currDir = upDir(currDir,cmd)
        currMv = mvDir[currDir]
        
    else:
        for ii in range(cmd):
            nxtPos = [currPos[0]+currMv[0], currPos[1]+currMv[1]]
            if tuple(nxtPos) in obstacles:
                break
            else:
                currPos = nxtPos
