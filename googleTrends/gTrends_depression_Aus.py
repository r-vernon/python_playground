...
#%% read in the data - Australian edition!
# data downloaded relative to 2006, which had highest peak
...
for currYear in range(2006,2023):
    ...
    if currYear == 2006:
        ...
        # parse col1 as date
        df = (pd.read_csv('./data_Aus/Depression_%d.csv' % (currYear),skiprows=2,
...
if saveF: plt.savefig('./Fig1_Aus.png',dpi=150, pad_inches=0)
plt.show()
...
# save the figure and show
if saveF: plt.savefig('./Fig2_Aus.png',dpi=150, pad_inches=0)
plt.show()
...
# based on adj. R^2 - stillkeeping 3 peaks
#   first alone is ~23% var., then add about 11% [1], then 5-6% [2]
...
# save the figure and show
if saveF: plt.savefig('./Fig3_Aus.png',dpi=150, pad_inches=0)
plt.show()

# the 3 peaks...
#   2, 1, 4 cycles per year in that order
#   likely some interesting things going on with harmonics
...
ax[0,0].set_title('2 Cycles/yr',fontdict={'fontsize':'small'},loc='left')
ax[0,0].set_ylim(-30,40)
ax[0,0].set_yticks(np.arange(-30,50,10))
...
ax[1,0].set_title('2+1+4 Cycles/yr',fontdict={'fontsize':'small'},loc='left')
ax[1,0].set_ylim(-30,40)
ax[1,0].set_yticks(np.arange(-30,50,10))
...
ax[0,1].set_yticks(np.arange(-15,20,5))
...
ax[1,1].set_yticks(np.arange(-15,20,5))
ax[1,1].set_xticks(xt, labels=xtl, fontsize='small', rotation=90.0, family='monospace')

# save the figure and show
if saveF: plt.savefig('./Fig4_Aus.png',dpi=150, pad_inches=0)
plt.show()
